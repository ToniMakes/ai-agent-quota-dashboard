...
Once approved: configure the secret/variables above, run the `Package Windows` GitHub Actions workflow with `sign_with_signpath` set to `true` from the intended release commit or tag, verify the signed installer's Authenticode signature, and upload only that artifact to a new signed follow-up release. Do not silently replace the unsigned `v0.1.0` asset.
